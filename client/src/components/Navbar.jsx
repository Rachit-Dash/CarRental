  ...
  return (
    ...
    `}>
        ...
        ${open ? "max-sm:translate-x-0" : "max-sm:translate-x-full"}`}>
            ...
            <div className='flex max-sm:flex-col itmes-start sm:items-center gap-6'>

                <button className='cursor-pointer' onClick={()=> isOwner ? navigate('/owner') :
                changeRole()}>{isOwner ? 'DashBoard' : 'List Cars'}</button>

                <button className='cursor-pointer px-8 py-2 bg-primary hover:bg-primary-dull 
                transition-all text-white rounded-lg' onClick={()=> {user ? logout() : 
                setShowLogin(true)}}>{user? 'Logout' : 'Login'}</button>
            </div>
        </div>
